...
for(s in 1:ns)
{
  fith<-hpp_lm(yS[,s],XS[,,s],lambda=lambdaS[s]) 
  fitl<-lqa_lm(yS[,s],XS[,,s],lambda=lambdaS[s])
...
for(s in 1:ns)
{
  objh<-hpp_lm(yS[,s],XS[,,s],lambda=lambdaS[s],itmin=iter,itmax=iter)$obj
  objl<-lqa_lm(yS[,s],XS[,,s],lambda=lambdaS[s],itmin=iter,itmax=iter)$obj
...
## -- plots
pdf(paste0("lm_l1p_results.pdf"),family="Times",height=3.5,width=8)
