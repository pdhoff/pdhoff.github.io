...
{ 
  ...
  while( (length(obj)<=itmin) | ( del>tol & length(obj)<=itmax) )
  {
    ...
    beta<-irls(y,X,beta=beta,family=family,ridge=W)
                  ...
                  beta=NULL,tol=1e-6,itmin=1,itmax=Inf) 
{
  ...
  while( (length(obj)<=itmin) | ( del>tol & length(obj)<=itmax) )
  {
    for(k in 1:K)
    {
      ...
      Xv<-sweep(X,2,v,"*") 
      U[,k]<-irls(y,Xv,beta=U[,k],family=family,ridge=ridge)
    }
    beta<-apply(U,1,prod) 
...
#### ---- 
irls<-function(y,X,family="binomial",ridge=diag(0,ncol(X)),tol=1e-6,beta=beta)
{
  if(family=="binomial"){ dA<-dA_bin ; ddA<-ddA_bin }
  if(family=="poisson"){ dA<-dA_pois ; ddA<-ddA_pois }
